...
func InitRedis(network, address string, password string) redis.Conn {
	// 缓冲机制，相当于消息队列
	if len(redisPoll) == 0 {
...
func redisServer2(w http.ResponseWriter, r *http.Request) {

	startTime := time.Now()
	c := InitRedis("tcp", "192.168.1.201:6379", "redis666")
	dbkey := "netgame:info"
	if _, err := c.Do("SET", dbkey, "yanetao"); err == nil {
